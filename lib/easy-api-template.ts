...
const authenticateApiKey = async (req: AuthenticatedRequest) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    ...
    return new NextResponse(JSON.stringify({ error: 'Bearer token is missing' }), { status: 401 });
  }
  ...
  if (!apiKey) {
    ...
    return new NextResponse(JSON.stringify({ error: 'Invalid Bearer token' }), { status: 401 });
  }
  ...
  try {
    ...
    if (apiKeyRecord.status !== 'active') {
      ...
      return new NextResponse(JSON.stringify({ error: 'API key is not active' }), { status: 401 });
    }
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        const updateResult = await tx.update(users)
          .set({ tokenBalance: Number(authenticatedReq.user!.tokenBalance) - options.tokenCost })
